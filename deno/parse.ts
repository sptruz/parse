...
import {
  ...
  TRANSPARENT_REGEX,
} from './constants/pattern.regex.ts';
import { getHEX, getHSL, getRGB } from './utils/getColor.util.ts';
import toLowerKeys from './utils/toLowerKeys.util.ts';
...
const parse = (str: string) => {
  ...
  const colorNameRGB = toLowerKeys(ColorName);
